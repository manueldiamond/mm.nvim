...
-- Load custom Bleach theme
vim.cmd.colorscheme 'bleach'
...
end, { desc = 'Open file directory in file manager' })
...
vim.api.nvim_create_user_command('NvimPush', function()
  ...
  local cmd = string.format('cd %s && git add . && git commit -m %s && git push --force', config_dir, commit_msg)
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>1', builtin.help_tags, { desc = '[S]earch [H]elp' })
      vim.keymap.set('n', '<leader>11', builtin.keymaps, { desc = '[S]earch [K]eymaps' })
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<leader><CR>', require('telescope.builtin').lsp_implementations, '[G]oto [I]mplementation')
          ...
          map('<leader>=', vim.lsp.buf.declaration, '[G]oto [D]eclaration')

          -- Fuzzy find all the symbols in your current document.
          --  Symbols are things like variables, functions, types, etc.
          -- Search variables
          map('<leader>v', require('telescope.builtin').lsp_document_symbols, 'Open Document Symbols')

          -- Fuzzy find all the symbols in your current workspace.
          --  Similar to document symbols, except searches over your entire project.
          --  Search workspace symbols
          map('<leader>vv', require('telescope.builtin').lsp_dynamic_workspace_symbols, 'Open Workspace Symbols')

          -- Jump to the type of the word under your cursor.
          --  Useful when you're not sure what type a variable is and you want to see
          --  the definition of its *type*, not where it was *defined*.
          map('<C-t>', require('telescope.builtin').lsp_type_definitions, '[G]oto [T]ype Definition')
  ...
  { -- Autocompletion
    ...
    version = '1.*',
    dependencies = {
      -- 1. Add Supermaven and the compatibility bridge
      'supermaven-inc/supermaven-nvim',
    ...
    opts = {
      keymap = {
        preset = 'default', -- Your existing preset
        -- This maps 'Enter' (CR) to accept the current selection
        ['<CR>'] = { 'accept', 'fallback' },
      },
      appearance = {
        nerd_font_variant = 'mono',
      ...
      completion = {
        ...
        -- TIP: Enable ghost text inside the blink menu to see AI suggestions inline
        ghost_text = { enabled = true },
      },

      sources = {
        -- 2. Add 'supermaven' to your default sources
        default = { 'lsp', 'path', 'snippets', 'lazydev', 'supermaven' },
        providers = {
          lazydev = { module = 'lazydev.integrations.blink', score_offset = 100 },
          -- 3. Define the supermaven provider using blink.compat
          supermaven = {
            name = 'supermaven',
            module = 'blink.compat.source',
            score_offset = 100, -- AI suggestions will show at the top
            async = true,
          },
        },
      },
